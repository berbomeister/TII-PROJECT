...
def perplexity(nmt, sourceTest, targetTest, batchSize):
    ...
    return math.exp(H/c)
def translate_and_rate(model,sourceFile,sourceTranslated,targetFile,bleu = False):
    sourceText = utils.readCorpus(sourceFile)
    model.eval()
    ...
    if bleu:
        ref = [[s] for s in utils.readCorpus(sourceTranslated)]
        hyp = utils.readCorpus(targetFile)
        print(len(ref))
        print(len(hyp))
...
if len(sys.argv)>2 and sys.argv[1] == 'trans':
    ...
    print(nmt.translateSentence(sentence))
